...
pub fn PUCT(inist : State, Cp : f64, maxExp :i32, heuristic_w :f64, verbose : bool, timeout : f64, registerName : String) -> State{
            ...
            }else
            {
                ...
                if moves.len() == 0 || st.terminal() {
                    ...
                    for mv in moves {
                        ...
                        let mut PUCT_score = f64::NEG_INFINITY;

                        if nombre_visite_fils != 0 {


                            //UCT_score = ave_fils/bestScore + Cp * (2.0* (nombre_visite_pere as f64).ln() / (nombre_visite_fils as f64) ).sqrt(); //UCT
                            PUCT_score = ave_fils/bestScore + Cp * heuristic_w* st.heuristic(mv).exp()* ((nombre_visite_pere as f64).sqrt() / (nombre_visite_fils as f64 + 1.0) ); //exponentiation de heuristic car ln à la base (SMILEGEN)

                        }else{
                            PUCT_score = f64::INFINITY;
                        }
                        if best_UCT_score < PUCT_score || (best_UCT_score == PUCT_score && rand::random::<f64>() < 0.5 ) {

                            next_moveSeq = new_moveSeq.clone();
                            best_UCT_score = PUCT_score;
                            best_move = mv.clone();
                        }
                    }
...
pub fn launch_PUCT(init_state : State, Cp : f64, maxExp : i32, heuristic_w : f64,timeout : f64, registerName : String) -> State{
    //let mut inist = State::new();
    return PUCT(init_state, Cp, maxExp, heuristic_w, false, timeout, registerName);
}
